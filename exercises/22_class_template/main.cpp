...
struct Tensor4D {
    ...
    Tensor4D(unsigned int const shape_[4], T const *data_) {
        ...
        std::memcpy(this->data, data_, this->size * sizeof(T));
    }
    ~Tensor4D() {
        delete[] data;
    }
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        ...
        // broadcasting add for each of the elements
        for (unsigned int i = 0; i < this->size; i += 1) {
            unsigned int idx = i, other_idx = 0;
            unsigned int stride = 1, other_stride = 1;

            for (int dim = 3; dim >= 0; dim -= 1) {
                unsigned int pos = (idx / stride) % this->shape[dim];
            ...
            this->data[i] += others.data[other_idx];
        }
